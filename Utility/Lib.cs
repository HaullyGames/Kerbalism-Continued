...
namespace KERBALISM
{
  ...
  {
    #region UTILS ----------------------------------------------------------------
    public static readonly string NAME_LOG_PREFIX = "[Kerbalism] ";
    ...
    public static class KCOMMNET
    {
      public static Vessel CommNodeToVessel(CommNode node)
      {
        ...
        foreach (Vessel w in FlightGlobals.Vessels)
        {
          ...
          if (KCommNetwork.AreSame(node, w.connection.Comm))
          {
            return w;
          }
        }

        Verbose("The node {0} is not valid.", node.name);
        return null;
      }

      // Original code below by TaxiService from https://github.com/KSP-TaxiService/CommNetConstellation
      ...
      public static ModuleDeployablePart.DeployState String_to_DeployState(string state)
      {
        switch (state)
        {
          case "BROKEN": return ModuleDeployablePart.DeployState.BROKEN;
          case "EXTENDED": return ModuleDeployablePart.DeployState.EXTENDED;
          case "EXTENDING": return ModuleDeployablePart.DeployState.EXTENDING;
          case "RETRACTED": return ModuleDeployablePart.DeployState.RETRACTED;
          case "RETRACTING": return ModuleDeployablePart.DeployState.RETRACTING;
        };
        return ModuleDeployablePart.DeployState.EXTENDED;
      }
    }
    #endregion
